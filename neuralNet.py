    ...
    def build_model(self, X, Y, nn_hdim, iterations=10000):
        ...
        for i in range(iterations):
            ...
            if prt:
                ...
                print err[1:5,:]
            if i % 1000 == 0 or i <= 10: 
                loss, predict_error = self.getLoss_training(X, Y, w1, w2, b1, b2) #for all examples in a batch
            ...
            if prt:
                print "nl"
                print nl_delta
            l2_delta = np.dot(w2.T, nl_delta) * self.activation_delta(a2, "tanh") #hidden layer delta #transpose nl_delta?
            if prt:
                print "l2 delta"
                print l2_delta[1:5,:]
...
#             learningRate *= 0.9999
            lerningRate = 0.01 / (1 + 100*i/iterations)
